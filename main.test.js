...
function addNoises(arr) {
  ...
  for (const value of arr) {
    if (value === 'Fido' || value === 'Rolph' || value === 'Maisie') {
      newNoiseArray.push(value + ' says "Woof!\"');
    }
    else if (value === 'Garfield' || value === 'Heathcliff') {
      newNoiseArray.push(value + ' says "Meow!\"');
    }
    else if (value === 'Barnie' || value === 'Sharp Tooth') {
      newNoiseArray.push(value + ' says "ROWR.\"');
    }
...
function womenOnly(arr) {
  ...
  for (const value of arr) {
    ...
    if (value[value.length - 1] === 'F') { womenOnlyArray.push(value) }
  }
  return womenOnlyArray
}

function integersOnly(arr){
  let integersOnlyArray = []
  let isInteger = true
  for (const value of arr) {
      let str = value.toString()
          for (const charcters of str) {  
            if (charcters === '.'){ 
                  isInteger = false ;
                  break;
                } 
       ...
       if (isInteger === true ){ integersOnlyArray.push(value)}   
       isInteger = true;
  }
  return integersOnlyArray
}
/*********************************
